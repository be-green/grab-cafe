...
print(f"[OK] Total postings: {total}")

cursor.execute('SELECT COUNT(DISTINCT gradcafe_id) FROM postings')
unique_ids = cursor.fetchone()[0]
print(f"[OK] Unique GradCafe IDs: {unique_ids}")

if total == unique_ids:
    print("[OK] NO DUPLICATES - All postings have unique IDs")
else:
    print(f"[FAIL] WARNING: {total - unique_ids} duplicate IDs found!")
...
try:
    ...
    print(f"[OK] Successfully scraped {len(postings)} postings")
    ...
    if postings:
        ...
        if missing:
            ...
            print("[OK] All required fields present in scraped data")
...
print(f"[OK] posting_exists('{test_id}'): {exists_full}")

exists_recent = posting_exists_recent(test_id, days_back=7)
print(f"[OK] posting_exists_recent('{test_id}', 7 days): {exists_recent}")

fake_id = "999999999"
not_exists = posting_exists(fake_id)
print(f"[OK] posting_exists('{fake_id}'): {not_exists} (should be False)")
...
print(f"[OK] 100 recent checks: {elapsed_recent:.3f}s ({elapsed_recent/100*1000:.2f}ms each)")

start = time.time()
for _ in range(100):
    posting_exists(test_id)
elapsed_full = time.time() - start
print(f"[OK] 100 full checks: {elapsed_full:.3f}s ({elapsed_full/100*1000:.2f}ms each)")

speedup = elapsed_full / elapsed_recent
print(f"[OK] Recent check is {speedup:.1f}x faster")
...
if row:
    ...
    print("[OK] Sample formatted message:")
    print("-" * 40)
    print(message)
    print("-" * 40)
else:
    print("[FAIL] No posting with GPA and GRE found for formatting test")
...
print(f"[OK] Posted to Discord: {posted}")
print(f"[OK] Not yet posted: {unposted}")
print(f"[OK] Total: {posted + unposted}")

if posted + unposted == total:
    print("[OK] All postings accounted for")
else:
    print("[FAIL] Mismatch in posted status counts!")
